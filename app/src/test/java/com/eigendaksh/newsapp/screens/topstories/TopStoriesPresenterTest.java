...
public class TopStoriesPresenterTest {
    ...
    @Mock StoriesViewModel viewModel;
    @Mock NewsRequester newsRequester;
    @Mock Consumer<Boolean> loadingConsumer;
    @Mock Consumer<List<Story>> storiesUpdatedConsumer;
    ...
    public void storiesLoaded() throws Exception {
        ...
        initializePresenter();
        // Verify that when presenter is created getTopStories method gets called
        verify(newsRequester).getTopStories();
    ...
    private void initializePresenter() {
        presenter = new TopStoriesPresenter(viewModel, newsRequester);
    }
    ...
    private List<Story> setUpSuccess() {
        ...
        when(newsRequester.getTopStories()).thenReturn(Single.just(storyList));
        return storyList;
    }

    private Throwable setUpError() {
        Throwable error = new IOException();
        when(newsRequester.getTopStories()).thenReturn(Single.error(error));
        return error;
    }
}